...
export default memo(function XYDetailArtist(props) {
  ...
  useEffect(() => {
    document
      .getElementById('likeartist')
      .getElementsByClassName(
        'other-btn'
      )[0].firstElementChild.childNodes[1].data = '关注歌手';
  }, []);

  useEffect(() => {
    if (
      state.artistSongs &&
      state.artistSongs.artist &&
      state.artistSongs.artist.id
    ) {
  ...
  const zhName =
    state.artistSongs &&
    state.artistSongs.artist &&
    state.artistSongs.artist.name;
  const enName =
    state.artistSongs &&
    state.artistSongs.artist &&
    state.artistSongs.artist.alias[0];
  const artistPic =
    state.artistSongs &&
    state.artistSongs.artist &&
    state.artistSongs.artist.picUrl;
  return (
    <DetailArtistWrapper className="wrap-v2">
      <DetailArtistLeft>
        ...
        <div className="artistPic">
          <img
            src={getSizeImage(artistPic, 640, 300)}
            alt={zhName}
            className="show-img"
          />
        ...
        <ul className="menu">
          <li>
            <NavLink to={`/discover/detailartist/main?id=${showId}`}>
              热门作品
            </NavLink>
          </li>
          <li>
            <NavLink to={`/discover/detailartist/album?id=${showId}`}>
              所有专辑
            </NavLink>
          </li>
          <li>
            <NavLink to={`/discover/detailartist/mv?id=${showId}`}>
              相关MV
            </NavLink>
          </li>
          <li>
            <NavLink to={`/discover/detailartist/desc?id=${showId}`}>
              艺人介绍
            </NavLink>
          </li>
        </ul>
      ...
      <DetailArtistRight>
        <div className="simi-artist">
          ...
          <ul className="content">
            {state.artistSimi &&
              state.artistSimi.slice(0, 6).map(item => {
                return (
                  <li key={item.id} title={item.name}>
                    <NavLink
                      to={`/discover/detailartist/main?id=${item.id}`}
                      className="img"
                    >
                      <img
                    ...
                    <NavLink
                      to={`/discover/detailartist.mian?id=${item.id}`}
