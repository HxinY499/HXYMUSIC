...
import {SongTableYesAlbumWrapper} from './style'

export default memo(function XYSongTableYesAlbum(props) {
      ...
      <table className='songs'>
          <thead>
            <tr className='select'>
              ...
              <th className="text-nowrap">专辑</th>
            </tr>
          </thead>
          <tbody>
            {
                ...
                return (
                  <tr key={item.id}>
                    ...
                    <td className="text-nowrap">
                      ...
                      {item.alia&&item.alia.length>0 && <span style={{"marginLeft":"8px","color":"#aeaeae"}}>{" - "+item.alia[0]}</span>}
                      {
                    ...
                    <td className="text-nowrap">
                      <NavLink to={`/discover/detailartist/main?id=${(item.ar&&item.ar[0].id)||(item.artists&&item.artists[0].id)}`}>{(item.ar&&item.ar[0].name)||(item.artists&&item.artists[0].name)}</NavLink>
                    </td>
                    <td className="text-nowrap">
                      <NavLink to={`/discover/detailalbum?id=${(item.al&&item.al.id)||(item.album&&item.album.id)}`}>{(item.al&&item.al.name)||(item.album&&item.album.name)}</NavLink>
                    </td>
                  </tr>
                )
              })
