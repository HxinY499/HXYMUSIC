    ...
    <AppWrapper id="little-root">
      <BrowserRouter>
        ...
        <Suspense fallback={<Spin size="large" className="marin-load"></Spin>}>
          <div id="route-container">{renderRoutes(routes)}</div>
        </Suspense>
        <XYAppFooter />
        <div
          id={showPlayBar ? 'show-playBar-active' : ''}
          className={'show-playBar'}
        >
          <XYPlayBar />
        </div>
      </BrowserRouter>
      <BackTop>
        <div className="back-top main-button" title={'回到顶部'}>
          <UpOutlined />
        </div>
      </BackTop>
      <div
      ...
      <CSSTransition
        in={showSwitch}
        classNames="show-switch-playBar"
        timeout={1000}
        unmountOnExit={true}
      >
        <div
          className="switch-playbar main-button"
          onClick={() => {
            switchPlaybar();
          }}
