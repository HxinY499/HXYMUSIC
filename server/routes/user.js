...
router.post(
  '/changeAva',
  dir('./public/images/avatar').single('avatar'),
  function (req, res, next) {
    if (req.file) {
      const id = req.body.id;
      const newAvatar =
        'http://localhost:9002/static/images/avatar/' + req.file.filename;
      ...
      promiseOld.then(
        valueOld => {
          ...
          if (oldAvatar.substring(43) !== 'default_avatar.jpg') {
            fs.unlink(
              path.join('./public/images/avatar', oldAvatar.substring(43)),
              err => {
                if (err) {
                  console.log('旧头像删除失败');
                } else {
...
router.post('/register', function (req, res, next) {
  ...
  const avatar =
    'http://localhost:9002/static/images/avatar/default_avatar.jpg';
  ...
  promise.then(
    value => {
      if (value.length !== 0) {
        ...
        userModel.create(
          { id, username, password, nickname, avatar, registerTime },
          function (err, doc) {
            if (err) {
              console.log(err);
...
router.post('/basic', function (req, res, next) {
  ...
  const birthday = Date.parse(req.body.user.birthday)
    ? Date.parse(req.body.user.birthday)
    : Date.now();
  const residence = req.body.user.residence;
  userModel.updateOne(
    { id: id },
    { $set: { nickname, description, gender, birthday, residence } },
    function (err, raw) {
      if (err) {
        res.json({ data: { success: 'false', reason: err } });
      } else {
    ...
    promise.then(
      value => {
        if (value.length > 0) {
          ...
          res.json({
            data: { success: 'false', reason: constant.PASS_PROTECT_WRONG },
          });
        }
      },
      err => {
